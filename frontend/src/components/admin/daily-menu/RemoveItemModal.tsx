...
export function RemoveItemModal({
  ...
  return (
    <div className="fixed inset-0 bg-black/85 backdrop-blur-md z-50 flex items-start justify-center p-4 overflow-y-auto animate-in fade-in duration-200">
      <div className="bg-zinc-900 border border-zinc-800 rounded-3xl max-w-lg w-full mx-auto mt-20 p-6 shadow-2xl relative animate-in zoom-in duration-200">
        
        {/* Title */}
        <h3 className="text-xl font-extrabold text-zinc-100 flex items-center gap-2 mb-1">
          <span>⚠️</span> Deactivate Item
        </h3>
        <p className="text-xs text-zinc-400 mb-6">
          Removing <span className="text-amber-400 font-bold">{itemName}</span> from today's active menu. Please document the audit reason.
        </p>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
                ...
                return (
                  <button
                    ...
                    className={`p-3 rounded-xl border text-left flex items-start gap-3 transition-all cursor-pointer select-none group min-h-[64px] ${
                      isSelected
                        ? "bg-red-500/10 border-red-500/40 text-zinc-150"
                        : "bg-zinc-950/40 border-zinc-850 text-zinc-450 hover:border-zinc-800"
                    }`}
                  >
                    <span className="text-2xl pt-0.5 shrink-0">{opt.icon}</span>
                    <div>
                      <div className={`text-sm font-bold transition-colors ${isSelected ? "text-red-400" : "text-zinc-350 group-hover:text-zinc-200"}`}>
                        {opt.label}
                      </div>
                      <div className="text-[10px] text-zinc-550 leading-snug mt-0.5">
                        {opt.desc}
                      </div>
                    </div>
                  </button>
          ...
          <div className="space-y-2.5">
            <label htmlFor="reasonText" className="text-xs font-bold uppercase tracking-wider text-zinc-400">
            ...
            <textarea
              ...
              className="w-full bg-zinc-950/80 border border-zinc-850 rounded-xl px-4 py-3 text-sm text-zinc-200 placeholder-zinc-650 focus:outline-none focus:border-red-500/50 resize-none font-medium"
            />
            {errorText && (
          ...
          <div className="flex gap-3 pt-2">
            <button
              ...
              onClick={onCancel}
              className="flex-1 py-3 border border-zinc-800 text-zinc-400 hover:text-zinc-200 hover:bg-zinc-850 rounded-xl text-sm font-bold transition-all disabled:opacity-50 disabled:cursor-not-allowed bg-transparent cursor-pointer min-h-[48px]"
            >
              Cancel
            </button>
            <button
              type="submit"
              disabled={isSubmitting}
              className="flex-1 py-3 bg-red-600 hover:bg-red-500 text-zinc-50 rounded-xl text-sm font-bold transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-1.5 border-0 cursor-pointer min-h-[48px]"
            >
              {isSubmitting ? (
